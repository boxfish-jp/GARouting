...
def evalTSP(individual):
    distance = distance_map[individual[-1]][individual[0]] * (
        confusion_map[individual[-1]][individual[0]] + 1
    )
    for gene1, gene2 in zip(individual[0:-1], individual[1:]):
        distance += distance_map[gene1][gene2] * (confusion_map[gene1][gene2] + 1)
    return (distance,)
...
def main():
    ...
    pop, log = algorithms.eaSimple(
        pop, toolbox, 0.7, 0.2, 60, stats=stats, halloffame=hof
    )
    """
    # プロットの作成
    gen = log.select("gen")
